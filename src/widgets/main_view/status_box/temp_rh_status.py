...
class TempRhStatus(QWidget):
  def __init__(self):
    super(TempRhStatus, self).__init__()
    
    layout = QHBoxLayout()
    self.setLayout(layout)
    layout.setContentsMargins(0, 0, 0, 0)
    
    title = QLabel("Temperature/RH sensor... ")
    font = title.font()
    ...
    dispatcher.connect(self.set_status, signal = "set_temp_status", sender = dispatcher.Any)
